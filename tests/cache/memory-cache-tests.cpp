...
myrmo::cache::MemoryCache::Error imageExists(myrmo::cache::MemoryCache& cache, size_t pos, std::vector<char>* data)
{
	std::string file(images[pos].name);
	return cache.read(images[pos].name, data);
}

myrmo::cache::MemoryCache::Error insertImage(myrmo::cache::MemoryCache& cache, size_t pos)
{
	std::string file(images[pos].name);
...
myrmo::cache::MemoryCache::Error deleteImage(myrmo::cache::MemoryCache& cache, size_t pos)
{
...
void test_insert_read_delete_all_images()
{
	...
	{
		MemoryCache cache(myrmo::hash::sha1, new policy::LRU());
		std::vector<char> data;

		// Assert no images exist.
		for (size_t i = 0; i < IMAGE_COUNT; i++)
			MYRMO_ASSERT(imageExists(cache, i, &data) == MemoryCache::Error::ItemDoesNotExist);

		// Assert insertion of all images ok.
		for (size_t i = 0; i < IMAGE_COUNT; i++)
			MYRMO_ASSERT(insertImage(cache, i) == MemoryCache::Error::NoError);

		// Assert all images can be fetched.
		for (size_t i = 0; i < IMAGE_COUNT; i++)
			MYRMO_ASSERT(imageExists(cache, i, &data) == MemoryCache::Error::NoError);
		...
		for (size_t i = 0; i < IMAGE_COUNT; i++)
		{
			...
			MYRMO_ASSERT(cache.read(images[i].name, &data) == MemoryCache::Error::NoError);
		...
		{
			MYRMO_ASSERT(deleteImage(cache, i) == MemoryCache::Error::NoError);
		...
		MYRMO_ASSERT(cache.clear() == MemoryCache::Error::NoError);
...
void test_disk_cache_eviction_policy()
{
	...
	{
		MemoryCache cache(myrmo::hash::sha1, new policy::LRU(), cacheSizeInMiB); // 1 MiB disk cache
		std::vector<char> data;
		...
		for (int i = IMAGE_COUNT - 1; i >= 0; i--)
		{
			...
			{
				MYRMO_ASSERT(imageExists(cache, i, &data) == myrmo::cache::MemoryCache::Error::ItemDoesNotExist);
			}
			else
			{
				// Note: LRU order will be reversed because of this.
				MYRMO_ASSERT(imageExists(cache, i, &data) == myrmo::cache::MemoryCache::Error::NoError);
				++asserted;
			}
		}

		// Will evict images IMAGE_COUNT - 1 and IMAGE_COUNT - 2.
		MYRMO_ASSERT(insertImage(cache, 0) == MemoryCache::Error::NoError);
		MYRMO_ASSERT(cache.size() == 977682);
		MYRMO_ASSERT(cache.count() == 5);
		MYRMO_ASSERT(imageExists(cache, 0, &data) == myrmo::cache::MemoryCache::Error::NoError);
		MYRMO_ASSERT(imageExists(cache, IMAGE_COUNT - 1, &data) == myrmo::cache::MemoryCache::Error::ItemDoesNotExist);
		MYRMO_ASSERT(imageExists(cache, IMAGE_COUNT - 2, &data) == myrmo::cache::MemoryCache::Error::ItemDoesNotExist);
		MYRMO_ASSERT(imageExists(cache, IMAGE_COUNT - 3, &data) == myrmo::cache::MemoryCache::Error::NoError);

		// Will evict IMAGE_COUNT - 4 and IMAGE_COUNT - 5
		MYRMO_ASSERT(insertImage(cache, 7) == MemoryCache::Error::NoError);
		MYRMO_ASSERT(cache.size() == 892603);
		MYRMO_ASSERT(cache.count() == 4);
		MYRMO_ASSERT(imageExists(cache, 7, &data) == myrmo::cache::MemoryCache::Error::NoError);
		MYRMO_ASSERT(imageExists(cache, IMAGE_COUNT - 4, &data) == myrmo::cache::MemoryCache::Error::ItemDoesNotExist);
		MYRMO_ASSERT(imageExists(cache, IMAGE_COUNT - 5, &data) == myrmo::cache::MemoryCache::Error::ItemDoesNotExist);
		MYRMO_ASSERT(imageExists(cache, IMAGE_COUNT - 6, &data) == myrmo::cache::MemoryCache::Error::NoError);
